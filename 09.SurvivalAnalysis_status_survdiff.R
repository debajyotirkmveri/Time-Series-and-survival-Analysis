...
head(myCredit)   
View(myCredit)# Have a glimps of data
#=========================================================================
...
KM_G=survfit(Surv(Lifetime[Group!=1],Status[Group!=1])~Group[Group!=1])
plot(KM_G,col=c(1,2),xlab="time",ylab="survival probability",
     xlim=c(0,max(KM_G$time)+1),ylim=c(0,1.05))
legend('topright',c("G2=Skl","G3=HSkl"),lty=c(1,1),col=c(1,2),cex=.6)
title(main='Figure 6C',sub="KM Surv Fn") 
